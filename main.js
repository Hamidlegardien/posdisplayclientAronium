...
function initHttp() {
  ...
  httpSrv.listen(HTTP_PORT,'0.0.0.0',()=>{
    const ip=getIp();
    console.log(`\n╔════════════════════════════════════════╗`);
    console.log(`║   AroniumPOS Display Client v1.1.3     ║`);
    console.log(`╠════════════════════════════════════════╣`);
...
ipcMain.on('CREATE_COM_PAIR', (_,{port1,port2})=>{
  ...
  const logF   = path.join(tmpDir, 'vspe_install_log.txt');
  const wrapPS = path.join(tmpDir, 'vspe_wrap_' + Date.now() + '.ps1');
  const script = path.join(resDir, 'install_vspe_ports.ps1');

  try { fs.unlinkSync(logF); } catch {}

  const sl = script.replace(/\\/g,'\\\\').replace(/'/g,"''");
  const wrapLines = [
    `$scriptPath = '${sl}'`,
    `$psPort1    = '${port1}'`,
    `$psPort2    = '${port2}'`,
    `$psArgs     = "-ExecutionPolicy Bypass -NoProfile -NonInteractive -File \`"$scriptPath\`" -Port1 $psPort1 -Port2 $psPort2"`,
    `Start-Process powershell.exe -ArgumentList $psArgs -Verb RunAs`
  ];
  const bom  = Buffer.from([0xEF,0xBB,0xBF]);
  const body = Buffer.from(wrapLines.join('\r\n'), 'utf-8');
  fs.writeFileSync(wrapPS, Buffer.concat([bom, body]));

  exec(`powershell.exe -ExecutionPolicy Bypass -NoProfile -WindowStyle Hidden -File "${wrapPS}"`,
    {timeout:30000}, ()=>{});

  const started = Date.now();
  const poll = setInterval(()=>{
    try {
      const log = fs.readFileSync(logF, 'utf-8');
      const done = log.includes('=== SUCCESS:') || log.includes('=== FAILED');
      if (!done && Date.now() - started < 90000) return;
      clearInterval(poll);
      const success = log.includes('=== SUCCESS:');
      console.log('[VSPE] poll done. success:', success);
      try { fs.unlinkSync(wrapPS); } catch {}
      const win = setupWin||mainWin;
      if (win) win.webContents.send('COM_PAIR_RESULT',{
        success,
        error: success ? null : 'VSPE port creation failed. Check %TEMP%\\vspe_install_log.txt'
      });
    } catch(e) {
      if (Date.now() - started >= 90000) {
        ...
        try { fs.unlinkSync(wrapPS); } catch {}
        const win = setupWin||mainWin;
        if (win) win.webContents.send('COM_PAIR_RESULT',{success:false, error:'Timeout — UAC may have been cancelled.'});
      }
    }
  }, 1000);
});
