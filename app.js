...
class DisplayClient {
  ...
  _render(order) {
    ...
    // Notify media panel — fullscreen when no items, side panel when order active
    if (window._mediaSetOrderActive) {
      const hasItems = !order.welcome && (order.items||[]).length > 0;
      window._mediaSetOrderActive(hasItems);
    }
